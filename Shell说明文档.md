...
2. 根据空格来对于输入的字符串进行分割，以此来区分指令以及参数，使用`argv`数组进行存储，同时`ArgvIndex`参数用于存储分割的块数。如`"ls -a -l"`中，`argv[0]="ls", argv[1]="-a", argv[2]="-l", ArgvIndex=3`.
...
5. 第二块循环用于确定指令和指令的参数。需要将重定向符号前的指令提取出来，存储在`com`数组中，第一个对象为指令，后面的都是执行的参数。
...
1. 此段代码在`back==1`成立时才会执行（展示的代码中省略了判断语句）。`back`会在输入的命令的最后一个元素为`'&'`时才会被设置为1.
